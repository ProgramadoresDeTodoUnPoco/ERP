...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        if ($stmt->execute()) {
          echo '<script>alert("Donativo Registrado correctamente");</script>';
          // Redirigir a otra página después de mostrar el mensaje de éxito
          echo '<script>window.location.href = "/SYSGES/pages/ver-donaciones.php";</script>';
        } else {
            echo "Error al registrar el nuevo donativo: " . $stmt->errorInfo()[2];
        }
    } catch (PDOException $e) {
        // Manejar errores de manera adecuada
        echo '<script>alert("Error al registrar el nuevo donativo: ' . $e->getMessage() . '");</script>';
    }
