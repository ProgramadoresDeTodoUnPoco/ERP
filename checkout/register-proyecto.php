...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        if ($stmt->execute()) {
            echo '<script>alert("Nuevo proyecto registrado correctamente.");</script>';
            echo '<script>window.location.href = "/SYSGES/pages/ver-proyectos.php";</script>';
        } else {
            echo "Error al registrar el nuevo proyecto: " . $stmt->errorInfo()[2];
        }
    } catch (PDOException $e) {
        // Manejar errores de manera adecuada
        echo '<script>alert("Error al registrar el nuevo proyecto:' . $e->getMessage() . '");</script>';
    }
